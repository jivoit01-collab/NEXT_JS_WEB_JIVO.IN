...
export async function setBaruSahibAssociationSectionActiveAction(
  section: string,
  isActive: boolean,
): Promise<ActionResponse<PageContent>> {
  const guard = await requireAdmin<PageContent>();
  if (guard) return guard;
  try {
    const row = await setBaruSahibAssociationSectionActive(section, isActive);
    revalidatePath('/our-essence/baru-sahib-association');
    revalidatePath('/jivo-dev/our-essence-baru-sahib-association');
    return { success: true, data: row };
  } catch (err) {
    console.error('[setBaruSahibAssociationSectionActiveAction]', { section, err });
    return { success: false, error: 'Failed to update section visibility' };
  }
}

export async function reorderBaruSahibAssociationSectionsAction(
  ...
  try {
    await reorderBaruSahibAssociationSections(orderedSections);
    revalidatePath('/our-essence/baru-sahib-association');
    revalidatePath('/jivo-dev/our-essence-baru-sahib-association');
    return { success: true, data: null };
  } catch (err) {
    console.error('[reorderBaruSahibAssociationSectionsAction]', err);
