  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-10 mx-auto flex min-h-[600px] w-full max-w-7xl items-start px-5 py-14 text-center sm:min-h-[640px] sm:px-6 sm:py-16 lg:h-full lg:min-h-0 lg:max-w-none lg:px-0 lg:py-0"
      >
        <motion.div
          variants={revealItem}
          className="mx-auto w-full max-w-3xl text-white transition-transform duration-500 lg:absolute lg:top-[8.2%] lg:left-[52.4%] lg:w-[43vw] lg:max-w-[620px] lg:hover:-translate-y-1 xl:max-w-[680px] 2xl:max-w-[800px]"
        >
          <h2 className="font-jost-extrabold text-[clamp(2rem,3.05vw,4.25rem)] leading-none tracking-[0.02em] text-white uppercase drop-shadow-[0_3px_14px_rgba(0,0,0,0.36)] lg:whitespace-nowrap">
            {title}
          </h2>
          <p className="font-jost-medium mt-3 text-[10px] tracking-[0.22em] text-white/90 uppercase sm:text-xs 2xl:text-sm">
            {subtitle}
          </p>
          <p className="mx-auto mt-4 max-w-[590px] text-[clamp(0.82rem,0.92vw,1.02rem)] leading-relaxed text-pretty text-white/90 drop-shadow-[0_2px_10px_rgba(0,0,0,0.3)] 2xl:max-w-[720px]">
            {description}
          </p>
        </motion.div>
      </motion.div>
