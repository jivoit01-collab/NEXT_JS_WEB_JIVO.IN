...
import type { MustardOilsRangeContent, MustardProductVariant } from '../types';
import { defaultRangeContent } from '../data/defaults';
import { MUSTARD_ROSE, MUSTARD_CARD_CREAM, MUSTARD_LABEL } from '../constants';
...
interface Props {
  data?: MustardOilsRangeContent;
}

/** Section 2 — "MUSTARD OIL RANGE OF PRODUCTS": bottle cards on a rose field. */
export function RangeSection({ data }: Props) {
  ...
  return (
    <section
      aria-labelledby="mustard-range-heading"
      className="overflow-x-clip px-4 py-14 sm:px-6 sm:py-16 lg:px-8 lg:py-20"
      style={{ backgroundColor: MUSTARD_ROSE }}
    >
      <motion.div
        ...
        >
          <span className="relative inline-block">
            ...
            <span
              ...
              style={{ backgroundColor: MUSTARD_CARD_CREAM }}
            />
          </span>
        </motion.h2>
...
function VariantCard({
  ...
  const inner = (
    <>
      ...
      >
        ...
        <SafeImage
          src={image}
          alt={`Jivo cold pressed mustard oil — ${label}`}
          width={260}
          height={420}
          quality={85}
  ...
  if (href) {
    return (
      <Link
        href={href}
        aria-label={`View ${label} Jivo kachi ghani mustard oil`}
        className={`${cardClass} focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--mustard-rose)] focus-visible:outline-none`}
        style={{
          backgroundColor: MUSTARD_CARD_CREAM,
          ['--mustard-rose' as string]: MUSTARD_ROSE,
        }}
      >
        {inner}
      </Link>
  ...
  return (
    <figure className={cardClass} style={{ backgroundColor: MUSTARD_CARD_CREAM }}>
      {inner}
    </figure>
  );
}
