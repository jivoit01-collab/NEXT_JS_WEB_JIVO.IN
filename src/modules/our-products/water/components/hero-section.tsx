...
import type { WaterHeroContent } from '../types';
...
interface Props {
  data?: WaterHeroContent;
}

export function WaterHero({ data }: Props) {
  const {
    logoImage,
    heading,
    subtitleLineOne,
  ...
  return (
    <section
      ...
      // Same absolute overlay layout at EVERY size: logo top-centre, bottles
      // pinned bottom-right, and the heading/copy/CTA block centred over the
      // lower third — so mobile mirrors the desktop composition (just smaller).
      className="group relative isolate block w-full max-w-full overflow-hidden min-h-[74svh] sm:min-h-[72svh] lg:min-h-dvh"
      style={{ backgroundColor: WATER_NAVY }}
    >
      {/* Full-bleed background photo (mountain/lake). The navy shows only as a
          fallback behind it; a soft navy scrim keeps overlaid text legible. */}
      {!isPlaceholderValue(backgroundImage) ? (
        <>
          <SafeImage
            src={backgroundImage}
            alt=""
      ...
      <HeroBottles
        ...
        // vanished entirely.
        style={{
          // Bottle SIZE dial: raise the middle vw term to enlarge both bottles
          // together, the cap (rem) to let them keep growing on large monitors.
          width: 'clamp(14rem, 52vw, 92rem)',
      ...
      >
        ...
        <a
          ...
          className="
            ...
            hover:scale-[1.04]
            hover:bg-white
            hover:text-[color:var(--water-navy)]
            ...
            focus-visible:ring-offset-[color:var(--water-navy)]
...
export function WaterHeroSkeleton() {
  return (
    <section
