...
import {
  ...
  setWaterSectionActiveAction,
  reorderWaterSectionsAction,
} from '@/modules/our-products/water/actions';
...
export default function WaterManager() {
  ...
  const loadManagedSections = useCallback(async () => {
    const res = await getAllWaterSectionsAction();
    if (res.success) {
    ...
    <div className="space-y-4 sm:space-y-6 2xl:space-y-8">
      ...
      {managedSections.length > 0 && (
        <SectionManagerPanel
          ...
          onReorder={(orderedKeys) => reorderWaterSectionsAction(orderedKeys)}
          onToggleActive={async (key, isActive) => {
            const res = await setWaterSectionActiveAction(key, isActive);
