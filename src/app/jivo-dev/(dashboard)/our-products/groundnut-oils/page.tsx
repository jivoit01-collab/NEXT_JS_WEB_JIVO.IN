...
} from '@/modules/our-products/groundnut-oils/types';
import {
  defaultHeroContent,
  defaultRangeContent,
  defaultGoodnessContent,
  defaultAuthenticityContent,
} from '@/modules/our-products/groundnut-oils/data/defaults';

type ContentTabKey = 'hero' | 'range' | 'goodness' | 'authenticity';
type TabKey = ContentTabKey | 'seo';

const TABS: { key: TabKey; label: string }[] = [
  { key: 'hero', label: 'Hero' },
  { key: 'range', label: 'Range' },
  { key: 'goodness', label: 'Goodness Within' },
...
export default function GroundnutOilsManager() {
  ...
  const [hero, setHero] = useState<GroundnutOilsHeroContent>(defaultHeroContent);
  const [range, setRange] = useState<GroundnutOilsRangeContent>(defaultRangeContent);
  const [goodness, setGoodness] = useState<GroundnutOilsGoodnessContent>(defaultGoodnessContent);
    ...
    (async () => {
      try {
        ...
        if (json.success && json.data) {
          ...
          if (json.data.goodness)
            setGoodness({ ...defaultGoodnessContent, ...json.data.goodness });
          if (json.data.authenticity)
            setAuthenticity({ ...defaultAuthenticityContent, ...json.data.authenticity });
        }
      } catch (err) {
        console.error('[GroundnutOilsManager.load]', err);
  ...
  const handleSave = useCallback(async () => {
    ...
    const contentMap = { hero, range, goodness, authenticity };
    const content = contentMap[activeTab];

    const result = await upsertGroundnutOilsSectionAction(activeTab, content);
    ...
    setLoading(false);
  }, [activeTab, hero, range, goodness, authenticity]);
    ...
    <div className="space-y-4 sm:space-y-6 2xl:space-y-8">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          <h1 className="font-jost-bold text-xl sm:text-2xl md:text-3xl 2xl:text-4xl">
            Groundnut Oils — Page Manager
          </h1>
          <p className="mt-1 text-xs text-muted-foreground sm:text-sm 2xl:text-base">
            Manage the Our Products / Groundnut Oils page sections
          </p>
        </div>
      ...
      <div className="rounded-lg border bg-card">
        ...
        <div className="space-y-4 p-4 sm:space-y-6 sm:p-6 2xl:space-y-8 2xl:p-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={inputClass}
                  placeholder="Groundnut Oil carries the quiet strength of nature."
                />
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={inputClass}
                        placeholder="/our-products/groundnut-1l"
                      />
                    </div>
                    <div>
          ...
          {activeTab === 'goodness' && (
            <div className="space-y-6">
              <div>
                ...
                <input
                  type="text"
                  value={goodness.heading}
                  onChange={(e) => setGoodness({ ...goodness, heading: e.target.value })}
                  className={inputClass}
              ...
              <div>
                ...
                <textarea
                  value={goodness.paragraph}
                  onChange={(e) => setGoodness({ ...goodness, paragraph: e.target.value })}
                  rows={6}
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={goodness.benefitsHeading}
                  onChange={(e) => setGoodness({ ...goodness, benefitsHeading: e.target.value })}
                  className={inputClass}
              ...
              <StringListEditor
                label="Benefits"
                items={goodness.benefits}
                placeholder="Natural Extraction: Cold-pressed using traditional methods..."
                onChange={(benefits) => setGoodness({ ...goodness, benefits })}
              />

              <div>
                <label className={labelClass}>Section image (groundnuts)</label>
                <p className="mb-2 text-xs text-muted-foreground">
                  Shown to the right of the copy. A PNG/WebP with a transparent background sits
                  cleanest on the section colour.
                </p>
                <ImageUpload
                  value={goodness.image}
                  onChange={(url) => setGoodness({ ...goodness, image: url })}
                />
              </div>
            </div>
          )}
          ...
          {activeTab === 'authenticity' && (
            <div className="space-y-6">
              <div>
                ...
                <input
                  type="text"
                  value={authenticity.heading}
                  onChange={(e) =>
                    setAuthenticity({ ...authenticity, heading: e.target.value })
                  }
                  className={inputClass}
              ...
              <div>
                ...
                <textarea
                  value={authenticity.paragraph}
                  onChange={(e) =>
                    setAuthenticity({ ...authenticity, paragraph: e.target.value })
                  }
                  rows={5}
                  className={inputClass}
                />
              ...
              <div>
                <label className={labelClass}>Background image (groundnut field)</label>
                <p className="mb-2 text-xs text-muted-foreground">
                  Full-bleed behind the copy. A dark scrim is applied automatically so the text
                  stays readable.
                </p>
                <ImageUpload
                  value={authenticity.backgroundImage}
                  onChange={(url) => setAuthenticity({ ...authenticity, backgroundImage: url })}
                />
              </div>
            </div>
          )}
          ...
          {/* ── SEO Tab ────────────────────────────── */}
          {activeTab === 'seo' && <SeoTabPanel page="our-products-groundnut-oils" />}
        </div>
      </div>
