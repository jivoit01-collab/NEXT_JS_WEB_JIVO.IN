...
import {
  upsertForMotherEarthSectionAction,
  getAllForMotherEarthSectionsAction,
  setForMotherEarthSectionActiveAction,
  reorderForMotherEarthSectionsAction,
} from '@/modules/our-essence/for-mother-earth/actions';
...
export default function ForMotherEarthManager() {
  ...
  const loadManagedSections = useCallback(async () => {
    const res = await getAllForMotherEarthSectionsAction();
    if (res.success) {
    ...
    <div className="mx-auto w-full max-w-[1800px] space-y-6">
      ...
      {managedSections.length > 0 && (
        <SectionManagerPanel
          ...
          onReorder={(orderedKeys) => reorderForMotherEarthSectionsAction(orderedKeys)}
          onToggleActive={async (key, isActive) => {
            const res = await setForMotherEarthSectionActiveAction(key, isActive);
