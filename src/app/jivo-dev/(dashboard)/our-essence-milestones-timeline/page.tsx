...
import {
  ...
  setMilestonesTimelineSectionActiveAction,
  reorderMilestonesTimelineSectionsAction,
} from '@/modules/our-essence/milestones-timeline/actions';
...
export default function MilestonesTimelineManager() {
  ...
  const loadManagedSections = useCallback(async () => {
    const res = await getAllMilestonesTimelineSectionsAction();
    if (res.success) {
    ...
    <div className="mx-auto w-full max-w-[1800px] space-y-6">
      ...
      {managedSections.length > 0 && (
        <SectionManagerPanel
          ...
          onReorder={(orderedKeys) => reorderMilestonesTimelineSectionsAction(orderedKeys)}
          onToggleActive={async (key, isActive) => {
            const res = await setMilestonesTimelineSectionActiveAction(key, isActive);
